...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    assets: {
      css: {
        vendor: [
          ...
          '<%= bowerDir %>/snapjs/snap.css',
          '<%= bowerDir %>/angular-snap/angular-snap.css',
          '<%= bowerDir %>/angular-carousel/dist/angular-carousel.css',
          '<%= bowerDir %>/fuelux/dist/css/fuelux.css',
          //'<%= bowerDir %>/todo-theme/bootstrap.css',
          '<%= bowerDir %>/todo-theme/app.css',
          '<%= bowerDir %>/todo-theme/font.css',
          '<%= bowerDir %>/todo-theme/plugin.css',
          '<%= bowerDir %>/open-sans/css/open-sans.css',
      ...
      js: {
        vendor: [
          ...
          '<%= bowerDir %>/angular-snap/angular-snap.js',
          '<%= bowerDir %>/angular-cookies/angular-cookies.js',
          '<%= bowerDir %>/ngCordova/dist/ng-cordova.js',

          //firebase and dependencies were moved here to solve issues not being found
          '<%= bowerDir %>/firebase/firebase-debug.js',
          '<%= bowerDir %>/angularfire/dist/angularfire.js',
          //for next cleaning simple-login might go away   
          '<%= bowerDir %>/firebase-simple-login/firebase-simple-login.js',


          '<%= bowerDir %>/snapjs/snap.js',
          '<%= bowerDir %>/modernizr/modernizr.js',
          '<%= bowerDir %>/screenfull/dist/screenfull.js',
          '<%= bowerDir %>/fuelux/dist/loader.js',
          '<%= bowerDir %>/toastr/toastr.js',

          '<%= bowerDir %>/todo-theme/app.js',
          '<%= bowerDir %>/todo-theme/app.plugin.js',
          '<%= bowerDir %>/todo-theme/app.data.js',
          '<%= bowerDir %>/todo-theme/sortable/jquery.sortable.js',
    ...
    concat: {
      ...
      angular: {
        files: {
          ...
          '<%= tmpDir %>/js/utils.js'       : ['<%= srcDir %>/js/utils/*.js', '<%= srcDir %>/js/utils/**/*.js'],
          ...
          '<%= tmpDir %>/js/models.js'      : ['<%= srcDir %>/js/models/*.js', '<%= srcDir %>/js/models/**/*.js']
        }
      },
      production: {
        files: {
    ...
    copy: {
      ...
      fonts: {
        files: [
          ...
          { expand: true, cwd: '<%= bowerDir %>/bootstrap/dist/fonts/', src: ['**'], dest: '<%= appDir %>/font/' },
          { expand: true, cwd: '<%= bowerDir %>/open-sans/fonts/', src: ['**/**'], dest: '<%= appDir %>/font/open-sans/' }
        ]
      },
      tmp_to_build: {
        files: [
    ...
    ngmin: {
      ...
      utils: {
        src: '<%= tmpDir %>/js/utils.js',
        dest: '<%= tmpDir %>/js/utils.js'
      },
      controllers: {
      ...
      models: {
        src: '<%= tmpDir %>/js/models.js',
        dest: '<%= tmpDir %>/js/models.js'
      },
      modules: {
  ...
  grunt.registerMultiTask('config', '', function() {
